...
#Transforming the Date and Time vars from characters into objects of type Date and PoSIXlt repectively
subpower$Date<-as.Date(subpower$Date, format ="%d/%m/%Y")
subpower$Time<-strptime(subpower$Time, format = %H:%M:%S")
subpower[1:1440,"Time"format(subpower[1:1440,"Time"],"2007-02-02 %H:%M:%S)
subpower[1441:2880,"Time"]<-format(subpower[1441:2880, "Time"],"2007-02-02 %H:%M:%S")

#initiating a composite plot wuth many graphs
par(mfrow=c(2,2))
...
lengend("topright", lty=1, col ,"blue"), legend = c("Sub_metering_1","Sub_metering_2", "Sub_metering_3"),cex = 0,6)
